...
const App = () => {
  ...
  //  ошибка логина  //
  const [apiLoginError, setApiLoginError] = useState(defaultApiError);
  //  ошибка регистрации  //
  const [apiRegisterError, setApiRegisterError] = useState(defaultApiError);
  ...
  const getRegisterErrorMessage = (err) => {
    ...
    return err.message || 'Не удалось зарегистрироваться. Попробуйте ещё раз.';
  }

  const clearAuthErrors = () => {
    setApiLoginError(defaultApiError);
    setApiRegisterError(defaultApiError);
  }
  ...
  const handleLogin = (data) => {
    ...
    authLogin(data)
      ...
      .catch((err) => {
        ...
        setApiLoginError({
          isApiError: true,
          apiErrorMessage: getLoginErrorMessage(err),
        });
      });
  }
  ...
  const handleRegister = (data) => {
    ...
    authRegister(data)
      ...
      .catch((err) => {
        ...
        setApiRegisterError({
          isApiError: true,
          apiErrorMessage: getRegisterErrorMessage(err),
        });
      });
  }
          ...
          <Route path='/signin'>
            {loggedIn ? (
              ...
              <Login
                ...
                onAuthErrorReset={clearAuthErrors}
              />
            )}
          </Route>
